-- Two more purchasable piece sets — bauhaus and line — join the existing paid
-- sets in the shop at the same 40-token tier. Only the price list in
-- unlock_cosmetic() changes; the rest of the function is carried over verbatim
-- from 20260919140000_add_piece_style_shop_items.sql. Keep in sync with
...
begin
  ...
  v_price := case p_item_id
    ...
    when 'pieceSet:bauhaus' then 40
    when 'pieceSet:line' then 40
