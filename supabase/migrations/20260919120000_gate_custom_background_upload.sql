-- Custom background image uploads (the "Your Images" drag-and-drop on the
-- Shop page) used to be completely free and client-only (stored in
-- IndexedDB per-device). Now gated behind a one-time 40-token unlock, same
-- as every other cosmetic: pay once, own it, switching between owned things
-- stays free forever. Keep in sync with SHOP_ITEMS in src/data/shop.ts.
...
begin
  ...
  v_price := case p_item_id
    ...
    when 'background:custom' then 40
    else null
  end;
