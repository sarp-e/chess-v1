// Shared silhouette geometry for every paid piece set. All sets draw the same
// six shapes on the same 45x45 viewBox with the same footprint (base plinth at
// y=36-39) so they are recognisably "the same pieces, different finish" and
// drop into the board interchangeably.

export const PLINTH = <rect x="9" y="36" width="27" height="3" rx="1" />

// Some shapes need an internal detail (bishop's slit, knight's eye) that must
// contrast with the piece body — dark on a light piece, light on a dark one.
// A set's own stroke colour can't be relied on for these (a black-on-black
// stroke would vanish), so they take an explicit `accent` colour.
export const SHAPES: Record<'P' | 'R' | 'N' | 'B' | 'Q' | 'K', (accent: string) => React.ReactNode> = {
  P: () => (
    <>
...
export const PIECE_TYPES = ['P', 'R', 'N', 'B', 'Q', 'K'] as const
